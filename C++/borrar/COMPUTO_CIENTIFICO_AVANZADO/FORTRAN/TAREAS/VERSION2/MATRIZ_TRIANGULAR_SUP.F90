...
!Resolución de un sistema lineal AX=b, de matriz triangular inferior, 
...
!Declaración de una variable tipo fila
type(fila),dimension(:), allocatable::a
real, dimension(:), allocatable::b, x !array dinámico
integer::i,j,n, ios
real:: suma
...
!Lee la dimensión de la matriz
read(1,*,iostat=ios)n
!si no se encuentra el archivo o este esta vacío se produce un error
if (ios/=0) then
  close(1)
...
do i=1,n
  do j=1,n
   if (i>j) then
      read(1,fmt="(F5.2)",advance="no",iostat=ios)a(i)%r(j)!ojo: a la hora de crear
	  !el archivo, hay que considerar (ver formato) que cada elemento de la matriz
	  !debe tener 5 dígitos contando el punto y el espacio en blanco entre cada par
	  !de elementos, de lo contrario se obtendran resultados inesperados
      !NO SE PRODUCIRA SALTO DE LINEA HASTA LLEGAR AL ELEMENTO DIAGONAL 
	  if (ios/=0) stop "ERROR: error de lectura"!error de lectura
   else if(i==j) then
      read(1,*,iostat=ios)a(i)%r(j)!Cuando se encuentre con el elemento de la diagonal
	  !salta de línea
      if (ios/=0) stop "ERROR: error de lectura"!error de lectura
	  !ERROR: DIAGONALES = 0
	  if (a(i)%r(j)==0) stop "ERROR: Los elementos de la diagonal deben ser distintos de cero"
   end if
  end do
end do 
...
!***************************** VECTOR b ***********************************!
allocate(b(n))!reservo espacio para el array dinámico b
!LEO DEL ARCHIVO b
do i=1,n
...
allocate(x(n)) !reservo espacio para el vector solución x
x(1)=b(1)/a(1)%r(1) !primer elemento del vector solución
do i=2,n
   suma=b(i) 
   do j=1,i-1
